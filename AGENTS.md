...
```bash
mvn clean package           # full build (fat JAR at target/mvnminx-1.0.4.jar)
...
- Fat JAR output: `target/mvnminx-1.0.4.jar` (via maven-shade-plugin).
- `target/` should be gitignored.

## Architecture

- **`MvnMinEx.java`** — CLI entry point. Parses `--exclude`/`--exclude=` from args, strips them, wires `FilteringProjectRepository`, delegates to `MvnMinCli.run()`.
...
- **No mvnmin source is modified.** The wrapper uses composition over the `ProjectRepository` interface.
...
Version 1.0.4 is the initial release. Semantic versioning following mvnmin's release pattern.
